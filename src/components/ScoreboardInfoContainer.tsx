...
const ScoreboardInfoContainer: Component<{}> = (props) => {
    ...
    const swapPlayers = () => {
        const [player1, player2] = JSON.parse(JSON.stringify(state.scoreboard.players));
                ...
                <div class="flex gap-3 flex-wrap md:flex-nowrap">
                    <TextField>
                        <TextFieldInput
                            ...
                            onInput={(e) => update.scoreboard.update({ round: e.currentTarget.value })}
                        />
                    </TextField>
                    <TextField class="max-w-[50px]">
                        <TextFieldInput
                            type="text"
                            id="bestof"
                            placeholder="Best of"
                            value={state.scoreboard.bestof || ""}
                            onInput={(e) => update.scoreboard.update({ bestof: e.currentTarget.value })}
                        />
                    </TextField>
                    <TextField>
                        <TextFieldInput
                            ...
                            onInput={(e) => update.scoreboard.update({ tournamentName: e.currentTarget.value })}
                        />
                    </TextField>
                    <Button variant={"secondary"} class="min-w-[105px] flex-1" onClick={swapPlayers}>Swap Ports</Button>
                    <Button variant="destructive" onClick={() => { handleReset() }}>Reset</Button>
                    <Button class="min-w-[115px] flex-1 md:w-full" onClick={async () => await commitScoreboard()}>Send Update</Button>
                </div>
            </CardContent>
        </Card>
    );
